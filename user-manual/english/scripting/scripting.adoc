...
Scripts are entered as attributes of selected objects in the properties window.
...
In this window you write your script as described in the following sections. You can use the button btn:[Check] to let Antares check your script for syntactic or semantic errors. With the button btn:[OK] the current script is saved in the attribute where you opened the script editor.
...
Name:: The name or type of the issue. For scripting errors the name "Script Error". There are other types of errors, such as "Design Error", which occurs when your circuit contains wires connecting pins of different bit widths.
