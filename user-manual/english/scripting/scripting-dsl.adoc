...
Identifiers are used for naming an object that can hold a value, e.g. a variable (see below), or a reference to a component's pin, whose value would then be its current signal.

Names of identifiers must start with a letter, followed by an arbitrary number of letters, digits, or the character `_` (underscore).
...
[TIP]
====
Note that if you use `not` with number literals of type "Long" like `3`, the literal is first converted to a digital signal with the minimal required bit width, and only then inverted bit-wise. Therefore, `not 3` results in `0`.

This is to avoid running into signed inversion.  If that's not what you want, assign the number literal to a variable and apply the "Length cast" operator `$` (see below) when using the variable in the `not` term.
====
...
On the left side of the operator, only identifiers (variables) are allowed.

[source,javascript]
----
// Querying bit 2 of variable A with value 0b100 returns 1
A@2

// Use expressions for the bit position
A@(I + 1)

// Setting bit 0 in value A with value 0b100 returns 5 (binary 101)
A@0 = 1
----
