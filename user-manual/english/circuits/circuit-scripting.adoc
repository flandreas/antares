...
In a link:{{site.basedir}}/user-manual/english/circuits/circuits[circuit] a script can be defined in the attribute "Function" which is executed at link:{{site.basedir}}/user-manual/english/simulation.simulation["Flat simulation"] instead of the circuit if the circuit is used as a partial circuit in another circuit.
...
In the symbol of a link:{{site.basedir}}/user-manual/english/subcircuits/subcircuits[subcircuit] scripting can be used to change the display of the symbol during the simulation.
