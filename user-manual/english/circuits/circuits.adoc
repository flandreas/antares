...
Connecting components using lines is a separate, large topic that is dealt with in the separate chapter link:{{site.basedir}}/user-manual/english/circuits/wires["Wires"].

== Attributes of subcircuits

The properties of the built-in base components are explained in chapter link:{{site.basedir}}/user-manual/english/base-library/base-library["Base Library"]. This section describes the attributes of subcircuits which are inserted into a circuit.
