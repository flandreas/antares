...
From the inputs A and B, two wires each lead to different components. To create a junction of a wire from an existing wire, move the mouse over the existing wire and press the kbd:[Alt] key. Antares will then again indicate with a small orange circle that a new wire can be started here.
