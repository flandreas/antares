...
Use:: In this category the simulation is executed fastest. The wires do not display the signal by their color, and no animations are performed. This category is typically used to simulate complex circuits such as entire microprocessors that should execute a machine program as quickly as possible.
