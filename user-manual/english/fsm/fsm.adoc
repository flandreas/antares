...
:page-permalink: /user-manual/english/fsm
...
Output:: The output signal produced by the FSM when the system is in this state. Possible formats: "1" or "O=1", where "O" is the name of an output signal. If no output name is specified, all output definitions in the entire FSM must have no output name, and Antares applies the default name "O" to all of them.
...
Output:: The output signal produced by the FSM if it passes this transition (Mealy). Possible formats: "1" or "O=1", where "O" is the name of an output signal.
