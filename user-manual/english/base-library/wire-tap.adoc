...
Wide side pin:: The wire tap has a single input/output pin whose bit width is determined by attribute "Wide Bit Width".

Narrow side pins:: The wire taps has as many input/output pins at its narrow side indicates with attribute "Number of Outputs". Their bit width is determined by attribute "Narrow Bit Width".
...
Bit Position [n]:: The starting position of the bit range of the wide bus into which this narrow pin is tapping. These positions start with 0 indicating the least significant bit. The effective tapped bit range is the "Bit Position .. (Bit Position + Narrow Bit Width"). Example: Tapping into an 8 bit bus with narrow bit with 4 and bit position 2 taps the bits 2..6. +
+
...
[Note]
====
*Note:* Due to technical restrictions, Antares will reset the wire tap to standard settings when certain attributes like "Wide Bit Width" are changed.
====
