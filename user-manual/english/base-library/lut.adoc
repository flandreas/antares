...
A:: Address Input. Its bit width is determined by property "Address Bit Width".  If any of its bits is undefined (high impedance), the effective address is determined by the value of the user preference "Undefined gate input behaviour", e.g. "Read as 0" or "Read as random".
