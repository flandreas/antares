...
Magnitude values:: Certain components, especially the ones in the analog library, have properties that support magnitudes when entering a value, e.g. "100 K" for entering 100 kiloohm as resistance of a resistor, or "400 mA" for entering 400 milliampère as voltage of a battery. This is the list of supported magnitude characters:
+
- p: Pico
- n: Nano
- u: Micro
...
- G: Giga
+
If you omit the magnitude, such as when entering "480", the resolved default magnitude depends on the property. An electrical resistance would default to "480 Ohm", while a capacitor's capacity would default to "480 microfarad".
...
* image:user-manual/base-library/analog/led-fill.svg[LED] link:{{site.basedir}}/user-manual/english/base-library/analog/analog-led[LED]
