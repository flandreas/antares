...
If you connect a tunnel that doesn't have a name to a wire, Antares tries to derive a suitable name from the components already connected to the same net. For example, if you connect a new tunnel (has no name by default) to a clock with name "CLK", the tunnel also gets the name "CLK".
