...
Highlight current cell even without CS: Only displayed if the attribute "Show contents" is selected. Normally, the currently addressed
cell is only highlighted in the memory contents view if the memory is selected, i.e. the input CS is asserted. If this
attribute is set, the last addressed cell keeps highlighted even if the CS is not asserted any more. This can be
