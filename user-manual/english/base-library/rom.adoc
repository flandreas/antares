...
A:: Input "Address": Determines the address of the memory cell whose data is to be read out. If any of its bits is undefined (high impedance), the effective address is determined by the value of the user preference "Undefined gate input behaviour", e.g. "Read as 0" or "Read as random".
