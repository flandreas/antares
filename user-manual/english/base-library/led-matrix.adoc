...
Debug:: If this attribute is set, the LED matrix shows during simulation which rows and columns at the inputs have the value 1 at the moment, which can be helpful for debugging, but also for educational purposes, if e.g. a multiplexing circuit is developed.
