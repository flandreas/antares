...
=== Scrolling the signal curve

New signals always arrive at the right edge of each signal row, and then move towards the left edge when time passes, and eventually leave the visible area. The oscilloscope allows to scroll the visible area horizontally by clicking on a signal row and dragging horizontally. Double-clicking resets the scrolling offset.
...
Scaling:: Defines the amount by which the signal curve is scaled horizontally.

Mode:: Choose between "Real-Time" and "Clocked". See description in previous sections.

Nax. number of signal values:: Allows to overwrite the corresponding user preference setting for individual oscilloscopes.

Signal curve style:: The signal history contains of a series of values for individual moments in time. Drawing the signal curve can be done in two different ways:

- Rectangular: Two consecutive points are connected by two rectangular line segments.
- Diagonal: Two consecutive points are connected by a diagonal line

The default option is "Rectangular". For digital circuit, "Rectangular" is the only option, and the oscilloscope doesn't offer the "Signal curve style" property. Analog circuits with non-linear components (e.g. a LC oscillator), the "Diagonal" option leads to much smoother curves.
